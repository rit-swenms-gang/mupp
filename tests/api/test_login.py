...
class LoginResourceTest(TestCase):
    ...
    def test_login_successful(self):
        ...
        credentials = {"email": "login@fake.com", "password": "secure123"}
        res = test_post(
            self, base_url + "/login", json=credentials, expected_status=200
        )
        ...
        self.assertEqual(
            len(logins), 1, "Login entry should be created in logins table"
        )

    def test_login_fails_with_wrong_password(self):
        """POST to /login with invalid password returns 401 and does not add to logins table"""
        credentials = {"email": "login@fake.com", "password": "wrongpassword"}
        res = test_post(
            self, base_url + "/login", json=credentials, expected_status=401
        )
    ...
    def test_login_fails_with_nonexistent_user(self):
        ...
        credentials = {"email": "login@fake.com", "password": "doesntmatter"}
        res = test_post(
            self, base_url + "/login", json=credentials, expected_status=401
        )
        self.assertEqual(res["message"], "Invalid email or password")
    ...
    def test_logout_successful(self):
        ...
        login_res = test_post(
            self, base_url + "/login", json=login, expected_status=200
        )
        self.assertIn("session_key", login_res)

        before_logout = self.db.select(
            "SELECT * FROM logins WHERE user_id = %s;", ["1"]
        )
        self.assertEqual(
            len(before_logout), 1, "Logins table should have an entry before logout"
        )

        session_key = self.db.select(
            "SELECT session_key FROM logins WHERE user_id = %s;", ["1"]
        )
        logout_res = test_post(
            self,
            base_url + "/logout",
            header={"Session-Key": session_key[0][0]},
            expected_status=200,
        )
        ...
        self.assertEqual(
            len(after_logout), 0, "Logins table should be empty after logout"
        )
    ...
    def test_get_login_table_returns_data(self):
        ...
        login = {"email": "login@fake.com", "password": "secure123"}
        login_res = test_post(
            self, base_url + "/login", json=login, expected_status=200
        )
        ...
        self.assertGreaterEqual(
            len(res), 1, "Expected at least one login session in the response"
        )
