...
class TestMatchingWithFormSubmission(TestCase):
    ...
    def test_end_to_end_form_submission_to_matching(self):
        ...
        names = ["Kermit", "Miss Piggy", "Swedish Chef", "Pikachu", "Golden Freddy", "Freddy", "Bonnie", "Chica", "Foxy", "Snake"]
        for name in names:
        ...
        participants = [
            Participant(i['name'], i['email'], i['answers'])
            for i in rows
        ]

        generateMatches(self.leaders, participants, self.weights)
        tierListOptimizedGenerator(self.leaders, participants)
