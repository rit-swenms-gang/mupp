...
class Accounts(Resource):
    ...
    def post(self):
        ...
        parser.add_argument(
            "username", type=str, help="'username' must be a string", required=False
        )
        parser.add_argument(
            "email", type=str, help="'email' is a required property", required=True
        )
        parser.add_argument(
            "password",
            type=str,
            help="'password' is a required property",
            required=True,
        )
...
class Account(Resource):
    def get(self, account_id):
        db = Database(environ.get("DB_SCHEMA", "public"))
        try:
            accounts = db.tables["accounts"].select(
                ["username", "email"], {"id": account_id}
            )
            if len(accounts) == 1:
                return accounts[0], 200
    ...
    def put(self, account_id):
        ...
        parser = reqparse.RequestParser(bundle_errors=True)
        parser.add_argument(
            "username", type=str, help="'username' must be a string", required=True
        )
        parser.add_argument(
            "email", type=str, help="'email' is a required property", required=True
        )
